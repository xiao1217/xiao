...
if ($ltlx == "all"){
    ...
    if ($ltcxjg){
        ...
        $lthtml = "【世界|<a href='?cmd=$imliaotian'>私聊</a>|<a href='?cmd=$goliaotian'>刷新</a>】<br/>";
...
if ($ltlx == 'im'){
    ...
    if ($ltcxjg){
        ...
        $goliaotian = $encode->encode("cmd=liaotian&ltlx=all&sid=$sid");
        $imliaotian = $encode->encode("cmd=liaotian&ltlx=im&sid=$sid");
        $lthtml = "【<a href='?cmd=$goliaotian'>世界</a>|私聊|<a href='?cmd=$imliaotian'>刷新</a>】<br/>";
        for ($i=0;$i < count($ret);$i++){
            $uname = $ret[count($ret) - $i-1]['name'];
            $umsg = $ret[count($ret) - $i-1]['msg'];
            $uid = $ret[count($ret) - $i-1]['uid'];
            ...
            if ($uid){
                $lthtml .="<a href='?cmd=$ucmd'>$uname</a>-><a href='?cmd=$imucmd'>$uplayer->uname</a>:$umsg<br/>";
            }
        }
    }
    else{
